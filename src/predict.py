...
def main():
    ...
    # Run XLM-R test prediction in a separate python process using subprocess to guarantee VRAM cleanup
    import subprocess
    import sys
    ...
    cmd = [
        ...
        "test_df = pd.read_csv('dataset/processed/test_with_evidence.csv'); "
        "predict_test(test_df, config)",
    ]

    console.print("[cyan]Spawning subprocess for XLM-R prediction...[/cyan]")
    proc = subprocess.run(cmd)
    if proc.returncode != 0:
        raise RuntimeError("XLM-R test prediction subprocess failed.")

    # Load test predictions generated by the XLM-R subprocess
    preds_test_path = os.path.join(config["data"]["processed_dir"], "preds_test_xlmr.csv")
    if os.path.exists(preds_test_path):
        test_df = pd.read_csv(preds_test_path)
        p_xlmr = test_df["p_xlmr"].values
    else:
        raise RuntimeError(
            f"XLM-R test prediction subprocess failed: {preds_test_path} was not created."
        )
