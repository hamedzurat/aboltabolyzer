...
def main():
    ...
    # Run XLM-R CV in a separate process to guarantee GPU VRAM is completely released when done
    proc = multiprocessing.Process(target=train_cross_validation, args=(train_df, config))
    proc.start()
    proc.join()
